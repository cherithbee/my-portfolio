    ...
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 md:left-4 md:top-1/2 md:-translate-y-1/2 md:bottom-auto z-50 w-[95%] md:w-auto h-auto">
      <div className="flex flex-row md:flex-col items-center bg-white/10 dark:bg-black/40 backdrop-blur-2xl border border-white/20 p-1 md:p-2 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl transition-all duration-500">
        ...
        <div className="flex flex-row md:flex-col gap-1 w-full relative overflow-x-auto no-scrollbar md:overflow-visible pr-2 md:pr-0">
          ...
          {/* THE SLIDING LIQUID GLASS PILL - Fixed Desktop Width */}
          <div 
          ...
          {navLinks.map((link, index) => {
            ...
            return (
              <a
                ...
                className={`relative px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[11px] h-[40px] md:h-[52px] flex items-center justify-center uppercase tracking-[0.1em] md:tracking-[0.2em] font-bold transition-colors duration-300 z-10 text-center min-w-[90px] md:min-w-[140px] shrink-0 ${
