  ...
  return (
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 md:left-12 md:top-1/2 md:-translate-y-1/2 md:translate-x-0 md:bottom-auto z-[100] flex justify-center">
      <div className="flex flex-row md:flex-col items-center border border-black/[0.1] dark:border-white/10 shadow-2xl p-2 rounded-[3rem] backdrop-blur-2xl transition-all duration-500 
        /* FIX: Prevents the bar from expanding beyond the phone screen width */
        max-w-[92vw] md:max-w-none overflow-hidden">
        ...
        <button
          ...
          className="relative flex items-center justify-center w-10 h-10 md:w-12 md:h-12 my-2 md:my-4 mx-auto rounded-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 transition-all duration-300 active:scale-90 shrink-0 group"
        >
          {theme === 'dark' ? <Sun size={18} className="text-orange-400" /> : <Moon size={18} className="text-orange-600" />}
        </button>

        {/* SWIPEABLE LINK CONTAINER */}
        <div className="flex flex-row md:flex-col gap-1 relative overflow-x-auto md:overflow-visible no-scrollbar scroll-smooth h-auto py-1 flex-nowrap shrink">
          <div 
