...
export default function Navbar() {
  ...
  return (
    /* 1. POSITIONING: Top on mobile, Left side on Desktop */
    <nav className="fixed top-4 bottom-auto left-1/2 -translate-x-1/2 md:top-1/2 md:left-12 md:-translate-y-1/2 md:translate-x-0 md:bottom-auto z-[100] w-[95%] md:w-auto h-auto transition-all duration-500">
      ...
      <div className="flex flex-row md:flex-col items-center bg-white/40 dark:bg-black/40 backdrop-blur-[20px] backdrop-saturate-[180%] border border-white/30 dark:border-white/10 p-2 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl transition-all duration-500 min-h-fit overflow-visible">
        ...
        <div className="flex flex-col items-center px-4 md:px-0 md:pt-4 md:pb-2 border-r md:border-r-0 md:border-b border-black/10 dark:border-white/20 mr-2 md:mr-0 shrink-0">
          ...
          <span className="text-lg md:text-xl mt-1 text-gray-900 dark:text-white">🐝...</span>
        </div>
        ...
        >
          {theme === 'dark' ? <Sun size={18} className="text-orange-400" /> : <Moon size={18} className="text-gray-600" />}
        </button>
        ...
        <div className="flex flex-row md:flex-col gap-1 w-full relative no-scrollbar md:overflow-visible overflow-x-auto h-auto">
          <div 
            ref={pillRef}
            className="absolute bg-white/30 dark:bg-orange-500/20 border border-white/40 dark:border-orange-500/40 rounded-3xl z-0 will-change-transform"
            style={{ 
          ...
          {navLinks.map((link, index) => {
            ...
            return (
              <button
                ...
                /* 2. TEXT COLOR: Dark in Light Mode, Gray/White in Dark Mode */
className={`relative px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-[11px] 
  ...
  ${activeSection === id 
    ? 'text-orange-600 dark:text-orange-500' // COLOR WHEN SELECTED
    : 'text-black dark:text-gray-400'       // COLOR WHEN NOT SELECTED (Fixes the white text)
  }`}
              >
                {link.name}
              </button>
